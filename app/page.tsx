'use client'

import { useEffect, useState } from 'react'
import Link from 'next/link'
import { supabase } from '@/lib/supabase'

type Candidate = {
  drug_name: string
  proposed_indication: string
  evidence_level: string
  plausibility_score: number | null
}

const EVIDENCE_COLORS: Record<string, string> = {
  high:        '#4a7c59',
  medium:      '#b07d2e',
  low:         '#a04040',
}

const EVIDENCE_LEVELS = ['high', 'medium', 'low']

export default function HomePage() {
  const [allRows, setAllRows]           = useState<Candidate[]>([])
  const [search, setSearch]             = useState('')
  const [evidenceFilter, setEvidence]   = useState('all')
  const [loading, setLoading]           = useState(true)
  const [error, setError]               = useState<string | null>(null)

  useEffect(() => {
    async function fetchAll() {
      const { data, error } = await supabase
        .from('candidates')
        .select('drug_name, proposed_indication, evidence_level, plausibility_score')
        .order('drug_name')

      if (error) { setError(error.message); setLoading(false); return }
      setAllRows(data ?? [])
      setLoading(false)
    }
    fetchAll()
  }, [])

  const filtered = allRows.filter(row => {
    const matchesSearch   = !search || row.drug_name?.toLowerCase().includes(search.toLowerCase())
    const matchesEvidence = evidenceFilter === 'all' || row.evidence_level === evidenceFilter
    return matchesSearch && matchesEvidence
  })

  return (
    <main style={{ maxWidth: 860, margin: '0 auto', padding: '48px 24px', fontFamily: 'system-ui, sans-serif' }}>

      {/* ── Hero ── */}
      <div style={{ marginBottom: 48 }}>
        <h1 style={{ fontSize: 28, fontWeight: 700, letterSpacing: '-0.5px', marginBottom: 8 }}>
          Veronica&apos;s Drug Repurposing<br />Discovery Engine
        </h1>
        <p style={{ color: '#666', fontSize: 14 }}>
          Computational hypotheses for drug repurposing &mdash;<br />
          powered by SIDER, PubMed, and Claude.
        </p>
      </div>

      {/* ── Filters ── */}
      <div style={{ display: 'flex', gap: 32, marginBottom: 40, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, letterSpacing: '0.08em', color: '#888', textTransform: 'uppercase' }}>
          drug name
          <input
            type="text"
            placeholder="e.g. metformin"
            value={search}
            onChange={e => setSearch(e.target.value)}
            style={{ border: 'none', borderBottom: '1px solid #ccc', outline: 'none', padding: '6px 0', width: '220px' }}
          />
        </label>

        <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, letterSpacing: '0.08em', color: '#888', textTransform: 'uppercase' }}>
          evidence level
          <select
            value={evidenceFilter}
            onChange={e => setEvidence(e.target.value)}
            style={{ border: 'none', borderBottom: '1px solid #ccc', outline: 'none', padding: '6px 0', cursor: 'pointer', color: evidenceFilter === 'all' ? '#888' : '#1a1a1a' }}
          >
            <option value="all">all</option>
            {EVIDENCE_LEVELS.map(level => (
              <option key={level} value={level}>{level}</option>
            ))}
          </select>
        </label>
      </div>

      {/* ── Results ── */}
      {loading && <p style={{ color: '#999', fontSize: 14 }}>loading...</p>}
      {error   && <p style={{ color: '#c00',  fontSize: 14 }}>error: {error}</p>}

      {!loading && !error && (
        <>
          {/* Column headers */}
          <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr 100px 70px', gap: 12, padding: '0 0 8px', borderBottom: '1px solid #eee', fontSize: 11, letterSpacing: '0.08em', color: '#aaa', textTransform: 'uppercase' }}>
            <span>drug</span>
            <span>indication</span>
            <span style={{ textAlign: 'center' }}>evidence</span>
            <span style={{ textAlign: 'right' }}>score</span>
          </div>

          {filtered.length === 0 ? (
            <p style={{ marginTop: 24, color: '#999', fontSize: 14 }}>no results</p>
          ) : (
            filtered.map((row, i) => (
              <Link
                key={i}
                href={`/drug/${encodeURIComponent(row.drug_name)}`}
                style={{ display: 'grid', gridTemplateColumns: '160px 1fr 100px 70px', gap: 12, padding: '14px 0', borderBottom: '1px solid #f0f0f0', textDecoration: 'none', color: 'inherit', alignItems: 'center' }}
              >
                <span style={{ fontWeight: 600, fontSize: 14 }}>{row.drug_name}</span>
                <span style={{ fontSize: 14, color: '#444' }}>{row.proposed_indication || '—'}</span>
                <span style={{ textAlign: 'center' }}>
                  <span style={{
                    display: 'inline-block',
                    width: 12, height: 12,
                    borderRadius: '50%',
                    background: EVIDENCE_COLORS[row.evidence_level] ?? '#ccc',
                    verticalAlign: 'middle',
                    marginRight: 6,
                  }} />
                  <span style={{ fontSize: 12, color: '#666' }}>{row.evidence_level || '—'}</span>
                </span>
                <span style={{ fontSize: 14, textAlign: 'right', color: '#333' }}>
                  {row.plausibility_score != null ? row.plausibility_score : '—'}
                </span>
              </Link>
            ))
          )}

          <p style={{ marginTop: 16, fontSize: 12, color: '#bbb' }}>{filtered.length} result{filtered.length !== 1 ? 's' : ''}</p>
        </>
      )}

    </main>
  )
}
